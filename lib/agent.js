    ...
    var installServiceConnector = function (service, cb) {
        ...
        try  {
            ...
            readInstalled(".", null, null, function (err, data) {
                ...
                var dep = data.dependencies[service.connector.name]

                winston.debug("dependency: " + (dep ? dep.name + "@" + dep.version + " link: " + dep.link : "not found."));
                ...
                if (service.connector.version === 'link') { 
                    ...
                    if (dep && dep.version === service.connector.version) {
                        winston.info("Module " + target + " was installed.");
                        return cb();
                    };
                    npmInstall(target);
                }
