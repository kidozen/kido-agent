...
var logTET = function (label, startTimestamp) {
    var endTimestamp = Date.now(),
        duration = Moment.duration(endTimestamp - startTimestamp),
        hh = duration.hours(),
        mm = duration.minutes(),
        ss = duration.seconds(),
        ms = duration.milliseconds(),
        formatedDuration = (hh < 10 ? "0" + hh : hh) + ":" +
            (mm < 10 ? "0" + mm : mm) + ":" +
            (ss < 10 ? "0" + ss : ss) + ":" +
            (ms < 100 ? "00" + ms : ms);
...
module.exports = function () {
    ...
    var hookEvents = function (client) {
        ...
        on("connect", function () {
            ...
            // waits for the response event, 'registered'
        });
        ...
        on("invoke", function (command) {
            try {
                ...
                var cbConnector = function (err, data, headers) {
                    ...
                    logTET("Method invocation", startTimestamp);
                };
                ...
                service.instance.lookupMethod(command.data.method, function (err, func) {
                    if (err) {
                        ...
                        logTET("lookupMethod for " + service.name, lookupMethodStartTimestamp);
                        return emit("invalidMethod", command);
                    }
        ...
        on("getLog", function(data){
            ...
            if(data && data.data && data.data.from && 
                data.data.till && typeof data.data.from === 'number' && typeof data.data.till === 'number') {
                from = data.data.from;
                till = data.data.till;
            } else {
                from = 0;
            ...
            ss(client).emit('log-stream', socketStream, {
                id: data.id
            });

            fstream.Reader({ 'path': path.resolve(process.cwd(),"./logs/"), 'type': 'Directory' })
            .pipe(tar.Pack())
            .pipe(zlib.Gzip())
            .pipe(socketStream);
        });
    ...
    var installServiceConnector = function (service, cb) {
        ...
        getDependency(service.connector.name, function (err, dep) {
            ...
            var isRepositoryURL = function (version) {
                return version.indexOf("http") === 0 ||
                    version.indexOf("git" === 0);
            };

            var isCustomConnector = function (service) {
                return service.enterpriseApi === "custom" || 
                service.connectorName === "custom";
            };
    ...
    var createServiceIConnectorInstance = function (service, cb) {
        ...
        getDependency(service.connector.name, function (err, dep) {
            ...
            try {
                ...
                logTET("Constructor [" + service.connector.name + "]", startTimestamp);
