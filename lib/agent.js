...
module.exports = Agent = function () {
    ...
    /*
     * Encode an input into a Base64 String.
     * 'input' could be anything that a Buffer accepts
     */
    encodeToBase64 = function (input) {
        var encodedString = new Buffer(input).toString('base64');
        return encodedString;
    },

    getConnectOptions = function (config) {
        var kidoTokenB64 = encodeToBase64(config.kidoToken);
        var connectOptions = { secure: true, query: "name=" + config.name + "&" +
            "kidoToken=" + kidoTokenB64 + "&" +
            "version=" + packageJson.version
        };
        return connectOptions;
    },

    connectToHub = function (config) {
        winston.debug("Connecting to Hub");
        var connectOptions = getConnectOptions(config);
        winston.debug("Connect options", connectOptions);
        client = io.connect(config.wsEndpoint, connectOptions);
        winston.debug("Hooking events");
        hookEvents();
    },
    ...
    validateOptions = function (options) {
        ...
        validatedOptions.tenant = validatedOptions.tenant || configuration.get("tenant");
    ...
    loadNPM = function (cb) {
        ...
        npm.load (function (err, instance) {
            ...
            // sets prefix config
            var prefix = path.dirname(process.execPath);
            winston.debug("Valid prefix: ", prefix);
            npm.commands.config(["get", "prefix"], function (err, currentPrefix) {
                if (err) return cb(Error.create("Couldn't get NPM configuration prefix.", err));

                winston.debug("Current prefix: ", currentPrefix);
                if (currentPrefix === prefix) return cb();

                // updates prefix
                npm.commands.config(["set", "prefix", prefix], function (err) {
                    if (err) return cb(Error.create("Couldn't update NPM configuration prefix .", err));
            
                    winston.info("NPM's configuration prefix was updated to: " + prefix);
                    cb();
                });
            });
        });
    ...
    disposeInstance = function (service, cb) {
    ...
    hookEvents = function () {
        ...
        on("error", function (e) {
            ...
            if (e === "handshake unauthorized") {
                winston.info("Obtaining new KidoToken and reconnecting");
                self.authenticate(function (err) {
                    if (err) {
                        winston.error("Error reconnecting.", err);
                        return;
                    }
                    connectToHub(self.config);
                })
                return;
            }
        process.exit(process.exitCode);
    ...
    getConnectorLatestVersion = function (target, cb) {
        winston.info("Checking version of: " + target);
        npm.commands.show ([target], function (err, data) {
    ...
    installServiceConnector = function (service, cb) {
        try  {
            ...
            getDependency(service.connector.name, function (err, dep) {
                ...
                if (service.connector.version === 'link') { 
                ...
                } else if (service.connectorName === "custom") {
                    ...
                    var target = service.connector.name + "@latest";
                    getConnectorLatestVersion(target, function latestVersionHandler(err, latestVersion){
                        if (err) { return cb(Error.create("Failing getting latest version of ", { target: target }, err)); }
    ...
    createServiceIConnectorInstance = function (service, cb) {
