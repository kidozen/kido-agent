...
module.exports = Agent = function() {
    ...
    this.initialize = function (options, cb) {
        ...
        try {
            winston.info("Initializing with following options:", options);
    ...
    this.start = function (cb) {
        ...
        try { 
            ...
            client = io.connect(self.config.wsEndpoint,  connectOptions);
            hookEvents();
            cb();
        } catch (e) {
    ...
    var on = function(eventName, cb) {
        ...
        client.on(eventName, function() {
            ...
            cb.apply(self, args); // TODO: replace self by the caller of the function.
        })
    };
    ...
    var hookEvents = function () {
        ...
        on("invoke", function (command) {
            ...
            try {
                ...
                if (!funcConnector) {
                    winston.warn("Method '" + command.data.method + "' was not found.");
                ...
                var cbConnector = function (err, data) {
                    // build response data
                    var result = { id: command.id, response: {} };

                    if (err) result.response.error = err instanceof Error ? err.message : err;
                ...
                try {
                    ...
                    funcConnector.call(this, command.data.args, cbConnector);
        ...
        try {
            ...
            installServiceConnector (service, function (err) {
                ...
                createServiceIConnectorInstance (service, function (err) { 
                    ...
                    winston.info("Service was added and is ready.");
    ...
    var installServiceConnector = function (service, cb) {
        try  {
            ...
            // validates the connector does not is local.
            if (service.connector.version === 'link') { 

                //Links the package
                npm.commands.link ([service.connector.name], function (err) {
                    if (err) return cb(Error.create("'npm link' command failed.", { target: service.connector.name }, err));
                    winston.info("Module linked: " + service.connector.name);
                    cb();
                });
    ...
    var createServiceIConnectorInstance = function (service, cb) {
        ...
        try {
            ...
            var connectorClass = require(service.connector.name);
    ...
    var emitServiceError = function (message, id, service, err) {
        ...
        emit("serviceError", { id: id, service: service.name, err: newError });
        addingService = false;
    };
