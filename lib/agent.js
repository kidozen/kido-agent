    ...
    var hookEvents = function (client) {
        ...
        on("getLog", function(data){
            ...
            fs.readdir(path.resolve(__dirname,'/logs'), function(err, files){
                if(err) {
                    return winston.error(err.message, err);
                }

                async.eachSeries(files, function(file, callback) {
                    fs.createReadStream(path.resolve(__dirname,'/logs/', file))
                    .pipe(split())
                    .pipe(new ParseLogLine({
                        from: from,
                        till: till
                    }))
                    .pipe(socketStream)
                    ...
                    winston.info('Log files processed correctly');
                });
            });
        });
    };
