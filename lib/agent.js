...
module.exports = function () {
    ...
    var hookEvents = function (client) {
        ...
        on("addService", ({ id, data }) => addService(id, data));
        ...
        on("disconnect", () => winston.warn("Connection disconected!"));

        // id, err, data, headers, eventName        
        on("update", function ({ data, id }) {
            try {
                update(data, function (err, result) {
                    emitCommand(id, err, result);
                });
        ...
        on("restart", function ({ data, id }) {
            try {
                restart(data, function (err, result) {
                    emitCommand(id, err, result);
                });
        ...
        on("getLog", function ({ data, id }) {
            try {
                getLog(data, function (err, result, headers, eventName) {
                    emitCommand(id, err, result, headers, eventName);
                });
    ...
    var addService = function (id, service) {
        ...
        if (addingService) {
            ...
            setTimeout(() => addService(id, service), 1000);
            return;
        }
        addingService = true;
        try {
    ...
    var createServiceConnectorInstance = function (service, cb) {
        ...
        getDependency(service.connector.name, function (err, dep) {
            ...
            try {
                ...
                cb(Error.create("Couldn't create service's connector instance.", { service }, e));
            }
        });
    };
...
function update(data = { version: "latest"}, cb) {
    var options = {
...
function restart(data = { wait: 3000 }, cb) {
    winston.verbose("cmd restart");
    asyncRestart(data.wait);
    cb(null, { restarting: true, wait: data.wait });
};

function exec(data = { signal: "SIGKILL", args: [""], timeout: (3 * 60 * 1000)}, cb) {
    ...
    child.on('exit', function (code= null, signal) {
